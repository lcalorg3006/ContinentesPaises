...
const CountryDetailsScreen = ({ route }: any) => {
  ...
  if (error) return <Text>Error al cargar los detalles del país.</Text>;

  const countryDetails = data[0]; // Suponiendo que el primer elemento es el correcto

  return (
    <View>
      <Text>{countryDetails.name.common}</Text>
      <Text>{countryDetails.capital}</Text>
      <Text>{countryDetails.languages ? Object.values(countryDetails.languages).join(', ') : 'No disponible'}</Text>
      <Image source={{ uri: countryDetails.flags[0] }} style={{ width: 50, height: 30 }} />
      <MapView
        style={{ height: 300 }}
        initialRegion={{
